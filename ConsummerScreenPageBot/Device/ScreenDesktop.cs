...
namespace ConsummerScreenPageBot.Device
{
    ...
    public static class ScreenDesktop
    {
        ...
        /// <summary>
        /// Chụp ảnh trang web bằng cách chia thành nhiều segment (phần) và chụp từng phần bằng cách scroll đến từng vị trí - Desktop
        /// 
        ...
        private static void CaptureSegmentScreenshots(IWebDriver driver, string hostLabel, int segmentCount, long jpegQuality)
        {
            try
            {
                ...
                Console.WriteLine($"[Desktop Segment] Total height: {totalHeight}px, Viewport height: {viewportHeight}px, Segment height: {sliceHeight}px");
                ...
                for (int i = 0; i < segmentCount; i++)
                {
                    ...
                    Console.WriteLine($"[Desktop Segment] Segment {i + 1}/{segmentCount}: Scroll to Y={scrollY} (target segment at Y={targetY})");
                    ...
                    {
                        Console.WriteLine($"[Desktop Segment] Error scrolling to {scrollY}: {ex.Message}");
                    }
                    ...
                    {
                        Console.WriteLine($"[Desktop Segment] Error capturing segment {i + 1}: {ex.Message}");
                        continue;
                    }
                    
                    if (segmentBytes == null || segmentBytes.Length == 0)
                    {
                        Console.WriteLine($"[Desktop Segment] WARNING: Segment {i + 1} screenshot is empty");
                        continue;
                    }
                    ...
                    try
                    {
                        ...
                        using (var img = Image.Load<Rgba32>(ms))
                        {
                            ...
                            Console.WriteLine($"[Desktop Segment] Saved segment {i + 1}/{segmentCount}: {Path.GetFileName(savePath)} ({compressedBytes.Length} bytes, {img.Width}x{img.Height}px)");
                    ...
                    {
                        Console.WriteLine($"[Desktop Segment] Error processing segment {i + 1}: {ex.Message}");
                        ErrorWriter.WriteLog(LogPath, "DesktopSegmentScreenshots.Process", $"Segment {i + 1} - {hostLabel} => {ex}");
                    }
                }
        ...
        private static byte[] CaptureViewportScreenshot(IWebDriver driver, long jpegQuality = 80)
        {
            ...
            {
                Console.WriteLine($"[Desktop] Lỗi khi chụp viewport screenshot: {ex.Message}");
            }
        ...
        /// <summary>
        /// Detect banner dạng chữ U ngược: banner trên + banner trái + banner phải
        /// </summary>
        private static bool DetectUShapedBannerLayout(IWebDriver driver, List<(int Left, int Top, int Right, int Bottom)> bannerRects, int pageWidth, out (int Left, int Top, int Right, int Bottom)? uShapeBounds)
        {
            ...
            try
            {
                ...
                foreach (var rect in bannerRects)
                {
                    ...
                    // Top banner: ở trên cùng (top < 500px), rộng >= 50% page width
                    if (rect.Top < 500 && width >= pageWidth * 0.5)
                    {
                        topBanners.Add(rect);
                    }
                    // Left sidebar: ở bên trái (left < 300px), không phải top banner
                    else if (rect.Left < 300 && rect.Top >= 200)
                    {
                        leftBanners.Add(rect);
                    }
                    // Right sidebar: ở bên phải (right > pageWidth - 300px), không phải top banner
                    else if (rect.Right > pageWidth - 300 && rect.Top >= 200)
                    {
                ...
                if (topBanners.Count > 0 && (leftBanners.Count > 0 || rightBanners.Count > 0))
                {
                    ...
                    // Chỉ coi là U shape nếu bottom >= 800px (đủ chiều cao để chụp viewport)
                    if (uBottom >= 800)
                    {
                        uShapeBounds = (uLeft, uTop, uRight, uBottom);
                        Console.WriteLine($"[Desktop] Detect U-shape banner: top={uTop}, bottom={uBottom}, left={uLeft}, right={uRight}");
                        return true;
                    }
                }
            }
            ...
            {
                Console.WriteLine($"[Desktop] Lỗi khi detect U-shape banner: {ex.Message}");
            }
        ...
        /// <summary>
        /// Gộp các banner gần nhau thành 1 ảnh để giảm số lượng request (tránh rate limit Gemini)
        /// 
        /// Logic:
            ...
            int mergeDistance = 150)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < sortedBanners.Count; i++)
                {
                    ...
                    if (group.Count > 1)
                    {
                        ...
                        Console.WriteLine($"[Desktop] Gộp {group.Count} banner gần nhau: ({group[0].Left},{group[0].Top}) -> ({minLeft},{minTop}) ~ ({maxRight},{maxBottom})");
                    }
                    else
                    {
                        merged.Add(current);
            ...
            {
                Console.WriteLine($"[Desktop] Lỗi khi merge banner: {ex.Message}");
        ...
        /// <summary>
        /// Detect các vùng banner trên trang và trả về danh sách tọa độ (left, top, right, bottom)
        /// </summary>
        private static List<(int Left, int Top, int Right, int Bottom)> DetectBannerRegions(IWebDriver driver)
        {
            ...
            try
            {
                ...
                // Detect iframes (thường là quảng cáo)
                var iframeScript = @"
                    const iframes = Array.from(document.querySelectorAll('iframe, frame'));
                    const minW = 120, minH = 30;
                ...
                // Detect banner elements bằng CSS selector
                ...
                Console.WriteLine($"[Desktop] Detect được {bannerRects.Count} vùng banner");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"[Desktop] Lỗi khi detect banner: {ex.Message}");
